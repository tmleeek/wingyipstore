...
class NBSSystem_Nitrogento_Model_Cache_Fullpage_Renderer_Factory
{
    ...
    protected function _retrieveStore()
    {
        ...
        $queryString = NBSSystem_Nitrogento_Helper_Url::getQueryString();
        ...
        if (!is_array($stores) 
         || strpos($queryString, '___store') !== false
         || strpos($queryString, '___from_store') !== false)
        {
            return false;
        }
    ...
    {
        if (NBSSystem_Nitrogento_Helper_Url::getRequest()->getParam(NBSSystem_Nitrogento_Helper_Data::NITROGENTO_REFRESH_CACHE_ON_PAGE))
        {
            return false;
        }
