 ...
 * @author     Wingyip <info@ebizmarts.com>
 */

class Wingyip_Realex_Model_Observer
{
    ...
    {
    	return Mage::getModel('realex2/realex_transaction');
    }
	...
	public function getSageSession()
    {
		if (is_null($this->_dbSess)) {
			$this->_dbSess = Mage::getModel('realex2/realex_session')->loadBySessionId(Mage::getSingleton('core/session')->getSessionId());
		}
	...
	{
		$sagepay = (bool)('Wingyip_Realex' == $controllerAction->getRequest()->getControllerModule());
		$config  = (bool)Mage::getStoreConfig('payment/realex/profile_request', Mage::app()->getStore());
	...
	public function profilePost($o)
	{
		...
		Mage::helper('realex')->logprofiler($ca);
	}

	public function layoutUpdate($o)
	{
		if(!Mage::helper('realex')->isSuiteEnabled()){
			return $o;
		}

		if(FALSE === Mage::helper('realex')->shouldAddChildLayout()){
			return $o;
		}

		$updates = $o->getEvent()->getUpdates();
		$updates->addChild('realex_checkout_review')
				->file = 'realex_checkout_review.xml';
	}
