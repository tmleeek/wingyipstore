...
class AW_Advancedreports_Model_Mysql4_Collection_Sales extends AW_Advancedreports_Model_Mysql4_Collection_Abstract
{
    ...
    public function reInitSelect($isAllStores = false)
    {
        ...
        $this->getSelect()
            ...
            # price
            ->columns(array('base_xprice' => "IFNULL(item2.base_price, item.base_price) * $currencyRate"))
            ->columns(array('base_original_xprice' => "IFNULL(item2.base_original_price, item.base_original_price) * $currencyRate"))
            # subtotal
            ->columns(
                array(
                    'base_row_subtotal' => "( IFNULL(item2.qty_ordered, item.qty_ordered) "
                        . "* IFNULL(item2.base_price, item.base_price) ) * $currencyRate"
                )
            )
            ...
            ->columns(
                array(
                    'base_xdiscount_amount' => "(main_table.base_discount_amount * $currencyRate)")
            )
            //tax
            ->columns(
                array(
                    'base_xtax_amount' => "(main_table.base_tax_amount * $currencyRate)")
            )
            # total
            ->columns(
                array(
            ...
            ->columns(
                array(
                    'base_row_xinvoiced' => "( IFNULL(item2.base_row_invoiced, item.base_row_invoiced) "
                        ...
                        . "- IFNULL(item2.base_discount_invoiced, item.base_discount_invoiced) ) * $currencyRate"
                )
            )
            ->columns(
                array(
            ...
            ->columns(
                array(
                    'base_row_xrefunded' => "( (IF((IFNULL(item2.qty_refunded, item.qty_refunded) > 0), 1, 0) "
                        ...
                        . "- ABS( IFNULL(item2.base_discount_amount, item.base_discount_amount) ) )  ) ) ) * $currencyRate"
                )
            )
            ->columns(
                array(
                    'base_tax_xrefunded' => "IF(( IFNULL(item2.qty_refunded, item.qty_refunded) > 0), ( IFNULL(item2.qty_refunded, item.qty_refunded) "
                        ...
                        . "*  IFNULL(item2.base_tax_invoiced, item.base_tax_invoiced) ), 0) * $currencyRate"
                )
            )
            ->columns(
                array(
