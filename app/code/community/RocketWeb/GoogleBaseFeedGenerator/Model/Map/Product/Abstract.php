    	...
    	if (!$this->getConfigVar('submit_no_img')) {
	    	...
	    	if (!$this->hasImage($this->getProduct())) {
	    		if ($this->getConfigVar('log_skip')) {
	    			$this->log(sprintf("product id %d product sku %s, skipped - product does not have an image.", $this->getProduct()->getId(), $this->getProduct()->getSku()));
	    		}
	    		$this->skip = true;
	    		return $this;
	    	}
...
//        if (!empty($value) && strpos($arr['attribute'], 'rw_gbase_directive_apparel') === 0 && $this->getGenerator()->getData('verbose')) {
//            echo('Apparel by '. $arr['column']. ' - '. $value. "\n");
//        }
    ...
    protected function mapDirectiveExpirationDate($params = array()) {
        return $this->getGenerator()->getData('expiration_date');
    }
    ...
    public function getGenerator() {
        return Mage::getSingleton('googlebasefeedgenerator/generator', array('store_code' => $this->getData('store_code')));
    }
}
