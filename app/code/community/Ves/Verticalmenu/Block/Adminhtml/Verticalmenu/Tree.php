...
class Ves_Verticalmenu_Block_Adminhtml_Verticalmenu_Tree extends Mage_Adminhtml_Block_Template {

    var $children = array();

    public function __construct() {
        parent::__construct();
        $this->setTemplate('ves_verticalmenu/verticalmenu/tree.phtml');
    ...
    protected function _prepareLayout() {
        $this->setChild('import_category_button', $this->getLayout()->createBlock('adminhtml/widget_button')
                        ->setData(array(
                            ...
                            'id' => 'add_subvesverticalmenu_button'
                        ))
        );
        ...
        $this->setChild('store_switcher', $this->getLayout()->createBlock('adminhtml/store_switcher')
                        ...
                        ->setTemplate('ves_verticalmenu/store/switcher.phtml')
        );
    ...
    public function getCloneLink() {
        ...
        if($store_id){
            return Mage::helper("adminhtml")->getUrl("*/adminhtml_verticalmenu/clone", array("store_id"=>$store_id));   
        }else{
            return Mage::helper("adminhtml")->getUrl("*/adminhtml_verticalmenu/clone");
        }
    }
    ...
    public function getRoot() {
        ...
        if (is_null($root)) {
            ...
            $rootIds = null;
            if ($storeId) {
                $rootIds = Mage::getModel('ves_verticalmenu/verticalmenu')->getRootId($storeId);
            } else {
                $rootIds = Mage::getModel('ves_verticalmenu/verticalmenu')->getRootId(0);
            }
            $root = array();
            foreach ($rootIds as $rootId) {
                $root[] = Mage::getModel('ves_verticalmenu/verticalmenu')->load($rootId);
            }
    ...
    protected function _getDefaultStoreId() {
        return Ves_Verticalmenu_Model_Verticalmenu::DEFAULT_STORE_ID;
    }

    public function getVerticalmenuCollection($id = null) {
        $storeId = $this->getStore();
        $storeId = (int)$storeId;
        $collection = $this->getData('verticalmenu_collection');
        if (is_null($collection)) {
            ...
            $this->setData('verticalmenu_collection', $collection);
        }
    ...
    public function getTreeHtml($id=null) {
        ...
        $childs = $this->getVerticalmenuCollection( $id );
    ...
    public function genTree( $parent, $level, $store_id = 0 ){
        if( $this->hasChild($parent) ){
            ...
            foreach( $data as $menu ){
                if($store_id){
                    $url  = Mage::helper("adminhtml")->getUrl("*/adminhtml_verticalmenu/index", array("id"=>$menu->getId(), "store_id"=>$store_id));   
                 }else{
                    $url  = Mage::helper("adminhtml")->getUrl("*/adminhtml_verticalmenu/index", array("id"=>$menu->getId()));
                }
                $class = "";
                $clone_link = "";
