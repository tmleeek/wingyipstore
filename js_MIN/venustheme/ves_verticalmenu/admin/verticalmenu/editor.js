...
(function($) {
	$.fn.PavVerticalmenuEditor = function(opts) {
		...
		function processMenu( item , _parent, _verticalmenu ){
		...
		/**
	 	 ...
	 	 */	
		function processSubMenu( item , _parent, _verticalmenu ){
		...
		/**
	 	 * listen Events to operator Elements of verticalmenu such as link, colum, row and Process Events of buttons of setting forms.
	 	 */	
		function listenEvents( $verticalmenu ){

			/**
			 *  Link Action Event Handler.
			 */
			$('.form-setting').hide();
			$( 'a', $verticalmenu ).click( function(event){
				
				var $this = this;
				var  $parent = $(this).parent();
				/* remove all current row and column are actived */
				$(".row", $verticalmenu).removeClass('active');
				$(".mega-col", $verticalmenu).removeClass('active');
				
			//	if( $parent.parent().hasClass('verticalmenu') ){
				 	var pos =  $(this).offset();
			...
			//	}
			 	...
			 	if( $parent.hasClass('dropdown-submenu') ){
			 		 ...
			 		 processSubMenu( $this, $parent, $verticalmenu );
			 	}else {   
			 		if( $parent.parent().hasClass('verticalmenu') ){
	                ...
	                $parent.addClass('open');
                 	...
                 	processMenu ( $this, $parent, $verticalmenu );
			 ...
			 */
			 ...
			 $($verticalmenu).delegate( '.row', 'click', function(e){ 
		 		$(".row",$verticalmenu).removeClass('active');
		 	...
		 	$($verticalmenu).delegate( '.mega-col', 'click', function(e){
 			 ...
 			 */
 			  ...
 			  */
 			  $("#unset-data-menu").click( function(){
 				 if( confirm('Are you sure to reset verticalmenu configuration') ){
 				    $.ajax({
 			  ...
 			  $($verticalmenu).delegate( '.ves-widget', 'hover', function(){ 
		 		//$(".row",$verticalmenu).removeClass('active');
		 		// $(this).addClass('active'); 
	    	 	 var w = $(this); 
		...
		function recalculateColsWidth(){
	 	...
	 	function saveMenuData(){
	 	 	var output = new Array();	
	 	 	 $("#verticalmenu-content #mainmenutop li.parent").each( function() {
				 	var data = $(this).data();
				 	data.rows = new Array();
	 	...
	 	function loadWidgets(){
	 		...
	 		$("#verticalmenu-content #mainmenutop .mega-col").each( function() {
	 		 	var col = $(this);		
	 			...
	 			$.ajax({
					...
					type:'POST',
			 		...
			 		if( ajaxCols.length == cnt ){
			 			$("#verticalmenu-content").delay(1000).fadeIn();
			 			$("#pavo-progress").delay(1000).fadeOut();
			 		}
		 			$( "a", col ).attr( 'href', '#verticalmenu-content' );
			   });	
	 		});
	 	}
	 	 ...
	 	 */	
	 	function reloadVerticalmenu(){

			var verticalmenu = $("#verticalmenu-content #mainmenutop");
			$( "a", verticalmenu ).attr( 'href', '#' );
			$( '[data-toggle="dropdown"]', verticalmenu ).attr('data-toggle','pavo-dropdown'); 
			listenEvents( verticalmenu );
			submenuForm();
			menuForm();
			loadWidgets();
	 	}
		...
		this.each(function() {  
			...
			$.ajax({
				url: config.action,
			 		...
			 		$("#verticalmenu-content").html( data );
			 		reloadVerticalmenu(  );
