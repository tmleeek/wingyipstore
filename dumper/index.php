...
if (!file_exists(PATH) && !$is_safe_mode) {
    mkdir(PATH, 0777) || trigger_error("Can't create backup catolog", E_USER_ERROR);
}
...
class dumper {
	...
	function backup() {
		...
		echo tpl_page(tpl_process("Creating BD copy"), $buttons);
		...
		mysql_select_db($db) or trigger_error ("Can't select data base.<BR>" . mysql_error(), E_USER_ERROR);
		$tables = array();
		...
		echo tpl_l("Creating reserv copy file...:<BR>\\n  -  {$this->filename}");
		...
		if ($this->mysql_version > 40101 && CHARSET != 'auto') {
			mysql_query("SET NAMES '" . CHARSET . "'") or trigger_error ("Can't change encoding of the connection.<BR>" . mysql_error(), E_USER_ERROR);
        ...
        foreach ($tables AS $table){
			...
			if ($this->mysql_version > 40101 && $tab_charset[$table] != $last_charset) {
				if (CHARSET == 'auto') {
					mysql_query("SET NAMES '" . $tab_charset[$table] . "'") or trigger_error ("Can't change encoding of the connection.<BR>" . mysql_error(), E_USER_ERROR);
					echo tpl_l("Connetion encoding set `" . $tab_charset[$table] . "`.", C_WARNING);
					$last_charset = $tab_charset[$table];
				}
				else{
					echo tpl_l('Encoding of connection and DB are different:', C_ERROR);
            ...
            while(($result = mysql_query("SELECT * FROM `{$table}` LIMIT {$from}, {$limit}")) && ($total = mysql_num_rows($result))){
                  ...
                  for($k = 0; $k < $fields; $k++){
                    if ($NumericColumn[$k])
                      ...
                      $funcName = function_exists('mysql_real_escape_string') ? 'mysql_real_escape_string' : 'mysql_escape_string';
                      $row[$k] = isset($row[$k]) ? "'" . $funcName($row[$k]) . "'" : "NULL";
                    }
        ...
        echo tpl_l(str_repeat("-", 60));
        $this->fn_close($fp);
		echo tpl_l("Copy of DB `{$db}` created.", C_RESULT);
		echo tpl_l("Size of DB:       " . round($this->size / 1048576, 2) . " Mb", C_RESULT);
		$filesize = round(filesize(PATH . $this->filename) / 1048576, 2) . " Mb";
		echo tpl_l("File size: {$filesize}", C_RESULT);
		echo tpl_l("Tables were processing: {$tabs}", C_RESULT);
		echo tpl_l("Strings were processing:   " . fn_int($tabinfo[0]), C_RESULT);
	...
	function restore(){
		...
		echo tpl_page(tpl_process("Restoring DB from reserv copy"), $buttons);
		...
		echo tpl_l("Connecting `{$db}`.");
		mysql_select_db($db) or trigger_error ("Can't select DB.<BR>" . mysql_error(), E_USER_ERROR);
		...
		if(preg_match("/^(.+?)\.sql(\.(bz2|gz))?$/", $file, $matches)) {
			...
			if (!file_exists(PATH . "/{$file}")) {
    		    echo tpl_l("Can't find file", C_ERROR);
				echo tpl_enableBack();
    		    exit;
    		}
		...
		if ($this->mysql_version > 40101 && (CHARSET != 'auto' || $this->forced_charset)) { // ��������� �� ���������, ���� � ����� �� ������� ���������
			mysql_query("SET NAMES '" . $this->restore_charset . "'") or trigger_error ("Can't change connection encoding.<BR>" . mysql_error(), E_USER_ERROR);
			echo tpl_l("Connection encoding `" . $this->restore_charset . "`.", C_WARNING);
			...
			if ($sql) {
			    if (preg_match("/;$/", $str)) {
						...
						elseif (preg_match("/CREATE TABLE/i", $sql)){
							...
							if (preg_match("/(CHARACTER SET|CHARSET)[=\s]+(\w+)/i", $sql, $charset)) {
								if (!$this->forced_charset && $charset[2] != $last_charset) {
									if (CHARSET == 'auto') {
										mysql_query("SET NAMES '" . $charset[2] . "'") or trigger_error ("Can't set connection Encoding.<BR>{$sql}<BR>" . mysql_error(), E_USER_ERROR);
										$cache .= tpl_l("Connection Encoding `" . $charset[2] . "`.", C_WARNING);
										$last_charset = $charset[2];
									}
									else{
										$cache .= tpl_l('Encoding of connection and DB are different:', C_ERROR);
							...
							elseif(CHARSET == 'auto'){ // ��������� ��������� ��� ������, ���� ��� �� ������� � ����������� auto ���������
								...
								if ($this->restore_charset != $last_charset) {
									mysql_query("SET NAMES '" . $this->restore_charset . "'") or trigger_error ("Can't set connection Encoding.<BR>{$sql}<BR>" . mysql_error(), E_USER_ERROR);
									$cache .= tpl_l("Connection encoding `" . $this->restore_charset . "`.", C_WARNING);
									$last_charset = $this->restore_charset;
								}
							}
						}
					...
					elseif($this->mysql_version > 40101 && empty($last_charset)) { // ������������� ��������� �� ������ ���� ����������� CREATE TABLE
						mysql_query("SET $this->restore_charset '" . $this->restore_charset . "'") or trigger_error ("Can't set connection Encoding.<BR>{$sql}<BR>" . mysql_error(), E_USER_ERROR);
						echo tpl_l("Connection encoding `" . $this->restore_charset . "`.", C_WARNING);
						$last_charset = $this->restore_charset;
					}
            		$insert = '';
		...
		echo tpl_s(1 , 1);
		echo tpl_l(str_repeat("-", 60));
		echo tpl_l("DB was restored from copy.", C_RESULT);
		if (isset($info[3])) echo tpl_l("Date of creation: {$info[3]}", C_RESULT);
		echo tpl_l("Num queries: {$q}", C_RESULT);
		echo tpl_l("Tables created: {$tabs}", C_RESULT);
		echo tpl_l("Strings inserted: {$aff_rows}", C_RESULT);
	...
	function main(){
		$this->comp_levels = array('9' => '9 (max)', '8' => '8', '7' => '7', '6' => '6', '5' => '5 (medium)', '4' => '4', '3' => '3', '2' => '2', '1' => '1 (min)','0' => 'no compression');
		...
		$buttons = "<INPUT TYPE=submit VALUE=Apply><INPUT TYPE=button VALUE=Exit onClick=\"location.href = 'index.php?reload'\">";
...
<TR><TD>Global Status:</TD><TD><TABLE WIDTH=100% BORDER=1 CELLSPACING=0 CELLPADDING=0>
...
<TABLE WIDTH=100% BORDER=0 CELLSPACING=0 CELLPADDING=2>
<TR>
<TD>We needed:<BR> - Internet Explorer 5.5+, Mozilla or Opera 8+ (<SPAN ID=sie>-</SPAN>)<BR> - JavaScript ON (<SPAN ID=sjs>-</SPAN>)</TD>
</TR>
</TABLE>
</FIELDSET>
</SPAN>
...
return <<<HTML
<CENTER>
<H1>Permission denied for this directory</H1>
</CENTER>
...
return <<<HTML
<FIELDSET>
<LEGEND>DB connecting error</LEGEND>
<TABLE WIDTH=100% BORDER=0 CELLSPACING=0 CELLPADDING=2>
<TR>
...
function SXD_errorHandler($errno, $errmsg, $filename, $linenum, $vars) {
	...
	echo tpl_l("{$dt}<BR><B>Some mistake...</B>", C_ERROR);
