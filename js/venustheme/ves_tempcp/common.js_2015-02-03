    ...
    jQuery.ajax({
        ...
        success: function(data){
            ...
            if(jQuery('.block-cart .btn-remove').length > 0){
                ...
                jQuery('.block-cart .btn-remove').unbind('click');
                jQuery('.block-cart .btn-remove').attr('onclick','');
                jQuery('.block-cart .btn-remove').off("click");
                jQuery('.block-cart .btn-remove').on('click', function(e){
                    e.preventDefault();
...
function addToCart( addtocartURL ) {
    if(isQuote){
        var result = confirm("By clicking OK below your current shopping basket will be emptied and replaced with the current item. If you do not wish for this to happen, click CANCEL.");
        if (result == false) 
            return false;
    }
...
$(window).ready( function(){
    ...
    $(".quantity-adder .add-action").click( function(){
       var input = $(this).parent('.quantity-wrapper').parent('.quantity-adder').children('.quantity-number').children('input');
    ...
    $(window).ready( function(){
        ...
        /*if(typeof(ajaxCart) != "undefined" && ajaxCart){*/
            ...
            if($('.block-cart .btn-remove').length > 0){
                ...
                $('.block-cart .btn-remove').unbind('click');
                $('.block-cart .btn-remove').attr('onclick','');
                $('.block-cart .btn-remove').on('click', function(e){
                    e.preventDefault();
                         ...
                         $.ajax({
                                ...
                                success: function(data){ 
                                    ...
                                    if (data.status == "SUCCESS") {
                                        ...
                                        jQuery('html, body').animate({ scrollTop: 0 }, 'slow'); 
                                        $('.cart-top .quick-access').addClass('quick-access-show');
                                        $('.cart-top .quick-access').addClass('quick-access-show-scrolled');
                                    }else{
                                        setLocation( url );
                                    }
                                } 
